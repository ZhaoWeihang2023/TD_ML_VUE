  ...
  <t-row :gutter="[24, 24]">
    <t-col :flex="3">
      <div class="user-left-greeting">
        <div>
          Hi，{{ useUserStore().userInfo.name }}
          <span class="regular"> {{ t('pages.user.markDay') }}</span>
        </div>
        <img src="@/assets/assets-tencent-logo.png" class="logo" alt="无法显示" />
      </div>

      <!-- 用户信息，页中 -->
      <t-card class="user-info-list" :title="t('pages.user.personalInfo.title')" :bordered="false">
        <template #actions>
          <t-button theme="default" shape="square" variant="text" @click="onEditInfo">
    ...
    <t-col :flex="1">
      <t-card class="user-intro" :bordered="false">
        <t-avatar size="80px">{{ useUserStore().userInfo.name[0] }}</t-avatar>
        <div class="name">{{ useUserStore().userInfo.name }}</div>
        <div class="position">{{ useUserStore().userInfo.roles[0] }}</div>
      </t-card>
